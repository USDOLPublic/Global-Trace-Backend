...
export class TranslateByKeyInterceptor implements NestInterceptor {
    ...
    private async translateNestedProperties(item, nestedKeys: string[], language: string) {
            ...
            if (Array.isArray(nestedProperty)) {
                item[property] = await Promise.all(
                    item[property].map(async (values) => ({
                        ...values,
                        [translatedKey]: await this.i18nService.translate(values[translatedKey], {
                            lang: language
                        })
                    }))
                );
            } else {
                item[property] = await this.i18nService.translate(nestedProperty[translatedKey], {
                    lang: language
                });
            }
        }
    ...
    async translate(data, context: ExecutionContext) {
        ...
        const normalKeys = this.reflector.get<string[]>('translateKeys', context.getHandler());
        const nestedKeys = this.reflector.get<string[]>('translateNestedKeys', context.getHandler());

        for (let key of normalKeys) {
            if (Array.isArray(data)) {
                for (let item of data) {
                    item[key] = await this.i18nService.translate(item[key], { lang: language });
                    await this.translateNestedProperties(item, nestedKeys, language);
                }
            } else {
                data[key] = await this.i18nService.translate(data[key], { lang: language });
            }
        }
        return data;
    }
