...
export class AddSelfAssessmentQuestionMillRole1655301829247 extends BaseMigration {
    part2 = 'part-2.json';
    part3 = 'part-3.json';
    ...
    getTemplatePart2Info() {
        const tempaltepart2Path = path.join(env.ROOT_PATH, `self-assessments/databases/data/mill/part-2.json`);
    ...
    getTemplatePart3Info() {
        const tempaltepart2Path = path.join(env.ROOT_PATH, `self-assessments/databases/data/mill/part-3.json`);
        if (fs.existsSync(tempaltepart2Path)) {
            return require(tempaltepart2Path);
        }
